...
export const SearchByVehicleHeroSection = ({
  onSearch,
  className,
  ...
  return (
    <div
      className={`flex flex-col md:flex-row max-w-[314px] items-start gap-4 md:gap-2.5 relative md:max-w-6xl w-full ${
        className && className
      }`}
    >
      {!dropdownLevels.length ? (
        <div className="flex flex-col md:flex-row w-full gap-1 md:gap-2.5">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="bg-gray-300 w-full block h-10 z-10 mb-3 last:mb-0 update-element-angle"
            />
          ))}
        </div>
      ) : (
          ...
          {dropdownLevels.map((level, index) => {
            ...
            return (
              <Select
                ...
                placeholder={`${level.typeName.toUpperCase()}`}
                className="update-element-angle py-2.5 mb-0"
      ...
      <div className="flex flex-row-reverse md:flex-row gap-2.5 w-full md:w-auto">
        <PrimaryButton
          content="SEARCH"
          className="leading-[150%] py-2 w-full md:w-fit px-3 lg:min-w-[144px] text-white update-element-angle"
        ...
        {AddClearButton && (
          <button
            ...
            className="w-full md:w-fit px-3 lg:min-w-[144px] h-10 cursor-pointer update-element-angle bg-white border border-[var(--color-secondary-300)] text-[var(--color-secondary-800)] font-semibold hover:bg-[var(--color-secondary-50)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-white"
          >
            CLEAR
          </button>
        )}
